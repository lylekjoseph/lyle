...
class LekkeslaapSpiderSpider(scrapy.Spider):
    ...
    def parse_listing(self, response):
        ...
        for t in range(1,11):
            ...
            supplierPage = response.xpath("//a[@class='name text']/@href")[t-1].extract()
            abSupPage = response.urljoin(supplierPage)
            yield Request(abSupPage)
            print(t)
            yield response.follow(supplierPage, callback=self.parse_book)
            with open('C:/Users/User/Desktop/lekkeslaap/lekkeslaap/spiders/24.05.20_test12.csv', "a") as fo:
                fo.write("\n"+str(fjson))
                fo.flush()
            #print(t)
