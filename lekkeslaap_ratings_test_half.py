...
class LekkeslaapSpiderSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.lekkeslaap.co.za/akkommodasie-in/kaapstad-middestad']
    ...
    def parse(self, response):
        ...
        for i in range(1,pages):
            print(i)
            next_page = 'https://www.lekkeslaap.co.za/akkommodasie-in/kaapstad-middestad/'+str(i)
    ...
    def parse_listing(self, response):
        ...
        for t in range(1,11):
            ...
            InnerPage = str(response.xpath("//a[@class='name text']/@href").extract_first())
            #print('#####################################################################################################################')
            #print(InnerPage)
            #InnerPageConcat = str(InnerPage) + '/resensies'
            #print(InnerPageConcat)


            yield response.follow(InnerPage, callback=self.parse_listing)
            fjson["Rating_Title2"] = response.css("#estabTabScroll span::text").extract_first()
            fjson["accrating2"] = response.css(".font-weight-bold.mr-1::text").extract_first()
        ...
        #print(title)
            ...
            with open('C:/Users/User/Desktop/lekkeslaap/lekkeslaap/spiders/lekkeslaap_ratingtest9.csv', "a") as fo:
                fo.write("\n"+str(fjson))
                fo.flush()
