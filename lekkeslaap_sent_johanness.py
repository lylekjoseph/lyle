...
class LekkeslaapSpiderSpider(scrapy.Spider):
    ...
    def parse_listing(self, response):
        ...
        for t in range(1,11):
            ...
            ''' ############ issue with this part. trying to go to supplier page and scrape ratings#################### , Index out of range error
            supplierPage = str(response.xpath("//a[@class='name text']/@href")[t-1].extract())
            yield response.follow(supplierPage, callback=self.parse_listing)
            ...
            print(t)
            '''
            with open('C:/Users/User/Desktop/lekkeslaap/lekkeslaap/spiders/johannes4.csv', "a") as fo:
                fo.write("\n"+str(fjson))
                fo.flush()
